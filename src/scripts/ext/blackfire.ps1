...
Function Add-Blackfire() {
  ...
  try {
    ...
    if (Test-Path $ext_dir\blackfire.dll) {
        ...
        $wc.DownloadFile("https://packages.blackfire.io/binaries/blackfire-php/${extension_version}/blackfire-php-windows_${arch}-php-${no_dot_version}${nts}.dll", "$ext_dir\blackfire.dll") > $null 2>&1
        Enable-PhpExtension -Extension blackfire -Path $php_dir
